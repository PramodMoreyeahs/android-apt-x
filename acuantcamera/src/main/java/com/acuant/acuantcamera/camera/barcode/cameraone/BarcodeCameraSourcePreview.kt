...
class BarcodeCameraSourcePreview(private val mContext: Context, attrs: AttributeSet?) : ViewGroup(mContext, attrs) {
    ...
    fun start(barcodeCameraSource: BarcodeCameraSource?) {
        if (barcodeCameraSource == null) {
            stop()
        }

        this.barcodeCameraSource = barcodeCameraSource
    ...
    private fun startIfReady() {
        if (mStartRequested && mSurfaceAvailable) {
            barcodeCameraSource!!.start(mSurfaceView.holder, this)
            mStartRequested = false
        }
    }
    ...
    override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
        ...
        if (barcodeCameraSource != null) {
            val size = barcodeCameraSource?.previewSize
            if (size != null) {
